...
```sh
$ git clone git://github.com/pasalo92/demoworks
$ cd demoworks
...
Finally, open `file:///path/to/demoworks/build/index.html` in your browser.

Happy hacking!

## Tech

`demoworks` contains a best-practice
directory structure to ensure code reusability and maximum scalability.
demoworks also comes prepackaged with the most popular design frameworks
...
getting `demoworks` up and running. Though `demoworks` is really simple
...
`demoworks` uses [Grunt](http://gruntjs.org) as its build system, so
...
```sh
$ git clone git://github.com/pasalo92/demoworks my-project-name
$ cd my-project-name
```
...
There are many Bower packages used by `demoworks`, like Twitter Bootstrap
...
Technically, `demoworks` is now ready to go.
...
`demoworks`. What follows in this section is a quick introduction to the
...
`demoworks` also includes [Live Reload](http://livereload.com/), so you no
...
default with `demoworks`) you need to tell Live Reload to allow it. Go to
